class PatternFound < RuntimeError
end

class PatternNotFound < RuntimeError
end

class Object
  # TODO: hacer explícito a que clase le están definiendo el "matches?" - Fixed by Santy
  #

  def matches?(obj, &b)
    PatternMatching.matched = PatternMatching.new(obj)

    begin
      PatternMatching.matched.instance_eval &b
    #    PatternMatching.new(obj).instance_eval &b
    rescue PatternFound
      # TODO: "matches?" tiene que retornar lo que retorne el bloque del pattern que matcheó - Fixed by Santy
...
class PatternMatching
  ...
  def list(a_list, match_size = true) # No me deja ponerle el ? a match_size? -- Mati
    ...
    Matcher.new (bindings) do |obj|
      cur_element = -1
      obj.is_a?(Array) && (!match_size || (a_list.length == obj.length)) &&
...
module Combinators
  def concat_bindings(*matchers)
    new_bindings = matchers.map(&:bindings)
    new_bindings.flatten!.concat bindings
    new_bindings
  end
  ...
  def and(*matchers)
    ...
    Matcher.new(new_bindings) { |obj| call(obj) && matchers.all?((proc { |matcher| matcher.call obj })) }
  end

  def or(*matchers)
    new_bindings = concat_bindings(*matchers)
    # TODO: lo mismo que para el "and"
    Matcher.new(new_bindings) { |obj| call(obj) || matchers.any? { |matcher| matcher.call obj } }
  end
